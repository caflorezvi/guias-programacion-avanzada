...
En este caso, `placesService.getAll()` devuelve una lista de alojamientos de prueba. Posteriormente, se llama a  `mapItemToMarker()` para convertir cada alojamiento en un `MarkerDTO`, que es el formato esperado por la función `drawMarkers()` del servicio de mapa. Finalmente, se dibujan los marcadores en el mapa utilizando `drawMarkers()`.
...
Puede ajustar las dimensiones del mapa según sus necesidades.

### 14. Implementación de `ngOnInit` en `CreatePlace`

Haga que la clase `CreatePlace` (`create-place.ts`) implemente `OnInit` y en la función `ngOnInit()` agregue lo siguiente:
...
La función `ngOnInit()` inicializa el mapa y agrega un marcador en respuesta a un clic. Posteriormente, actualiza las coordenadas de la ubicación del alojamiento en el formulario. Asegúrese de que el formulario tenga un campo llamado `location` que acepte un objeto con `latitude` y `longitude`.
