...
- **Lenguaje HTML**: Lenguaje de etiquetas de hipertexto usado para estructurar páginas web.
...
Observe que se está inyectando una variable `{{ title() }}` en el archivo `.html` que viene desde el controlador (`app.ts`) ya que dicho archivo tiene una clase con la propiedad `title` y el valor "**my-app**" (o el nombre que le haya dado a la aplicación).


### 10. Uso de variables en plantillas

Agregue otra variable en la clase `App` y úsela en el archivo `app.html`. La variable se puede llamar `footer` y asígnele el valor: `"Universidad del Quindío 2025"`, cambie el archivo HTML para que use dicha variable en lugar de tener el texto "quemado". Se puede basar en la variable `title`, que ya existe en dicha clase.
