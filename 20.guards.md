...
Para proteger las rutas podemos usar guards. **Cree una nueva carpeta que se llame guards (dentro de la carpeta app)** y ubicados dentro de dicha carpeta cree un archivo llamado `login-service.ts`. 
...
En este servicio se define la función `CanActivateFn`, la cual verifica si el usuario ya ha iniciado sesión. En caso afirmativo, se realiza una redirección a la página de inicio. Esto es útil porque un usuario autenticado no debería poder acceder a las páginas de login o registro, ya que están destinadas únicamente a usuarios no autenticados.
...
Si no la tiene, programe la función `getEmail()` en la clase `TokenService` (`token-service.ts`) así:
...
En la carpeta `guards` cree un archivo llamado `role-service.ts`. Este archivo contendrá la lógica para validar los roles de usuario al acceder a ciertas rutas.
...
Es necesario tener la función `getRole()` en el servicio `TokenService` para que el guard funcione correctamente. Observe que en este guard se valida si el usuario está logueado y si el rol del usuario coincide con alguno de los roles esperados para acceder a la ruta. En caso de que no cumpla con estas condiciones, se redirige al usuario a una página de acceso denegado (`/forbidden`).

> ⚠️ **Nota:** Asegúrese de tener una ruta configurada para `/forbidden` que muestre un mensaje adecuado al usuario cuando intente acceder a una página sin los permisos necesarios.
