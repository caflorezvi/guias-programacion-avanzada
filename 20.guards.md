...
Para proteger las rutas podemos usar guards. **Cree una nueva carpeta que se llame guards (dentro de la carpeta app)** y ubicados dentro de dicha carpeta escribimos lo siguiente en el cmd de windows o en la terminal de Linux o MacOS.

```bash
ng generate service login --skip-tests
```
...
Ahora modifique el archivo header.component.html en la sección del header donde está el botón de login y registro para que quede así:
...
### 18. Implementación de refresh token
...
Para esto, debe crear un nuevo endpoint de tipo POST con el nombre refresh `@PostMapping("/refresh")` en `AuthController` que permita actualizar el token vencido.

### 19. Función refresh en el frontend

En el frontend en `AuthService` (`auth-service.ts`) debe agregar otra función que reciba lo devuelto por el backend. Esta función se puede llamar `refresh()` y es prácticamente igual a la función `login()` solo que `refresh()` recibe un `TokenDTO` y apunta a la ruta `"/refresh"`. **Investigue cómo se puede modificar el interceptor para que cuando el backend nos retorna un error 401 se invoque la función `refresh()`, se actualice el token y envíe de nuevo la petición al backend con el nuevo token.**
