...
## Conceptos básicos
Bases de datos NoSQL MongoDB, Java, archivos JSON.
...
- Verificar que tiene instalado el JDK de Java (preferiblemente la versión 17).
...
Cree un DTO llamado `EmailDTO` que tenga el **subject, body y recipient**. Todos de tipo `String`.
...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendMail(EmailDTO emailDTO) throws Exception {
        Email email = EmailBuilder.startingBlank()
            .from("SMTP_USERNAME")
            .to(emailDTO.getRecipient())
            .withSubject(emailDTO.getSubject())
            .withPlainText(emailDTO.getBody())
            .buildEmail();
...
2. Active la autenticación de dos pasos: https://myaccount.google.com/signinoptions/two-step-verification/enroll-welcome
3. Genere una clave de aplicación: https://myaccount.google.com/apppasswords (esta clave es el SMTP_PASSWORD)
...
>**⚠️ IMPORTANTE:** Lo más aconsejable es que estos valores no estén "quemados" dentro de la clase sino que estén en un archivo de propiedades o en las variables de entorno del computador.
...
Este servicio debe ser incluido en aquellos métodos de los demás servicios que lo requieran.
...
1. Cree una cuenta en https://cloudinary.com/
2. Inicie sesión e ingrese a la consola: https://cloudinary.com/console/
...
- Implemente al menos un servicio adicional en su proyecto utilizando una API externa.