...
En esta guía trabajaremos en el **proyecto final del espacio acdémico**. Agregue la siguiente dependencia en el archivo `build.gradle`.
...
Cuando se haga el mapeo a la tabla `user`, se crearán las columnas correspondientes en la base de datos con las restricciones definidas en las anotaciones `@Column`. Como se puede observar, se han definido restricciones como `length` para la longitud máxima de los campos y `nullable` para indicar si un campo es obligatorio o no.
...
Con base en la especficación del proyecto final, cree la entidad `Place` con sus respectivos atributos y relaciones. Tenga en cuenta lo siguiente:
