...
Cree el DTO `EditUserDTO` con las validaciones pertinentes. Adicionalmente, cree el DTO `UserDTO`y `DetailUserDTO`, los cuales serán utilizados para devolver información al cliente. Dado que este último DTO es una respuesta generada por el backend, no requiere validaciones, ya que los datos ya fueron previamente procesados y verificados.
...
@PutMapping("/{id}")
public void edit(@PathVariable String id, @Valid @RequestBody EditUserDTO account) throws Exception{
...
@GetMapping("/{id}")
public DetailUserDTO get(@PathVariable String id) throws Exception{
    return null;
}
...
```java
@PutMapping("/{id}")
public ResponseEntity<ResponseDTO<String>> edit(@PathVariable String id, @Valid @RequestBody EditUserDTO account) throws Exception{
...
### Obtener todos los usuarios cuya ciudad de residencia es Armenia
GET http://localhost:8080/api/users?city=Armenia

### Obtener todos los usuarios cuya ciudad de residencia es Armenia y su nombre es Carlos
...
#### 21. Identificar rutas protegidas

Piense qué rutas deben estar protegidas (se requiere autenticación para usarlas). En este caso se debe especificar en el archivo `openapi.yaml`.

#### 22. Métodos de prueba

Para probar los diferentes métodos de los controladores podemos hacerlo de muchas maneras, en la próxima guía veremos tres: Postman, Swagger (OpenAPI) y pruebas unitarias con MockMvc. No es necesario que use los tres métodos para probar la API, con que use uno es suficiente.
