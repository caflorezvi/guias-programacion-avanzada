...
Generará una consulta para encontrar usuarios cuya edad sea mayor que el valor proporcionado. También están disponibles:
...
```
Creará una consulta para encontrar usuarios con un nombre específico y una edad dada. También están disponibles los operadores `Or` y `Not`.
...
Generará una consulta para encontrar usuarios cuyos nombres contengan la parte especificada.
...
```
Encontrará usuarios mayores que la edad proporcionada y los ordenará alfabéticamente por nombre ascendente.
...
```
Contará el número de usuarios con la edad dada.
...
```
En este ejemplo, se está utilizando una consulta SQL nativa para seleccionar usuarios cuya edad sea mayor que un valor específico. El atributo `nativeQuery = true` indica que la consulta es SQL nativa, por lo que se debe utilizar la sintaxis y las convenciones del motor de base de datos subyacente.
