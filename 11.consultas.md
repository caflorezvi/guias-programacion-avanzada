...
Cuando se utiliza Spring Boot en combinación con Spring Data JPA y una base de datos relacional como MariaDB, se puede aprovechar la configuración automática y las convenciones de Spring Boot para simplificar aún más el desarrollo de aplicaciones que interactúan con bases de datos.
...
Esto permitirá ordenar los resultados según los criterios especificados en el objeto `Sort`. Cuando se utiliza `Sort`, se puede definir el ordenamiento por uno o más campos, ya sea en orden ascendente o descendente, por ejemplo:
...
Este es un ejemplo de una consulta JPQL que selecciona todos los usuarios que tienen un nombre específico. Aunque aparece similar a SQL, JPQL opera sobre entidades y sus propiedades en lugar de tablas y columnas, por lo tanto es independiente del motor de base de datos.

Dado que `User` es una entidad, `u` es un alias para la entidad y `u.name` se refiere a la propiedad `name` de la entidad. Al usar JPQL, se **trabaja con el modelo de objetos de Java** en lugar de con la estructura de la base de datos.

Además, los parámetros en la consulta se indican con `:` seguido del nombre del parámetro.
...
Toda consulta se divide en dos partes principales: la cláusula `SELECT` y la cláusula `FROM`. La cláusula `WHERE` es opcional y se utiliza para filtrar los resultados según ciertas condiciones.
...
**Consulta con selección de campos específicos**
...
Retorna una lista de objetos `UserDTO` que contienen solo los campos `name` y `email` de los usuarios cuya edad es mayor que la especificada. Para que esto funcione, `UserDTO` debe tener un constructor que acepte los campos seleccionados o debe ser un `record` con los campos correspondientes.

```java
@Query("SELECT u.email FROM User u WHERE u.city = :city")
List<String> findNamesAndEmailsByCity(String city);
```

Retorna una lista de correos electrónicos de usuarios que viven en una ciudad específica. Dado que solo se selecciona el campo `email`, el tipo de retorno es `List<String>`.
...
En Spring Data JPA, la anotación `@Query` también permite definir consultas SQL nativas utilizando el atributo `nativeQuery = true`. Esto es útil cuando se necesita aprovechar características específicas del motor de base de datos o cuando la consulta es demasiado compleja para ser expresada en JPQL.

#### Ejemplo de consulta nativa

A diferencia de JPQL, las consultas nativas operan directamente sobre las tablas y columnas de la base de datos. Por eso, si quiero obtener todos los campos de aquellos usuarios cuya edad sea mayor que un valor específico, la consulta SQL nativa se vería así:
...
El atributo `nativeQuery = true` indica que la consulta es SQL nativa, por lo que se debe utilizar la sintaxis, las tablas y las columnas tal como están definidas en la base de datos relacional.
