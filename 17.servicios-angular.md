...
Los servicios se crean utilizando el comando `ng generate service nombre-del-servicio` y se **inyectan en los componentes mediante el constructor**. Esto permite que varios componentes utilicen la misma instancia del servicio, lo que facilita compartir datos y funcionalidades entre ellos.
...
export class PlacesService {
  ...
  public update(id: string, updatedPlace: Place) {
...
### 7. Creación de la interfaz `Place`

Es necesario crear la interfaz `Place` (recuerde ejecutar esto en la carpeta **models** del proyecto):

```bash
ng generate interface place
```

### 8. Definición de la interfaz `Place`
...
export class MyPlaces {
  ...
  constructor(public placesService: PlacesService) {
    this.places = placesService.getAll();
  }
...
En el componente de gestión de alojamientos (`my-places.ts`), agregue la función `onDelete(placeId: string)` que se ejecuta cuando el usuario hace clic en el botón de eliminar. Esta función debe llamar al servicio para eliminar el alojamiento y actualizar la lista. 

```typescript
public onDelete(placeId: string) {
...
export class DetailPlace {
  ...
  public get(placeID: string){
    const selectedPlace = this.placesServices.get(placeID);
