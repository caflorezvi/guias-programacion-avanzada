...
Puede agregar más alojamientos de prueba si lo desea. **Use ChatGPT para que le genere más alojamientos de prueba**, solo asegúrese que sigan la misma estructura, le puede pasar el ejemplo anterior para que los genere con base en ese formato.
...
> ⚠️ **Importante:** En esta guía, todas las operaciones se realizan utilizando un único DTO (`PlaceDTO`) para simplificar el proceso. Sin embargo, es importante recordar que en el backend los alojamientos cuentan con diferentes DTOs según los servicios específicos. Esto será más evidente al integrar el backend.
...
Una vez instalada, para usar la alerta **debe importarla a nivel la clase**. Se importa agregando lo siguiente:
...
Adicionalmente, se debe agregar una nueva ruta en el archivo `app.routes.ts` dentro de la lista `routes`:
...
> ⚠️ **Importante:** Asegúrese de que el último elemento de la lista sea siempre el que contiene la ruta `path: "**"`.
...
Recuerde que para usar la alerta **debe importarla a nivel la clase**, así como se hizo en el componente de gestión de alojamientos. Se importa así:
